...
В первую очередь для основного репозитория подключить набор пакетов (компоненту) contrib [^сcontrib] - zfs обитает там в виду невозможности включения в main из-за лицензионной несовместимости  
```
apt update
...
[^сcontrib]: nano /etc/apt/sources.list, в строке первого репозитория после main дописать contrib
